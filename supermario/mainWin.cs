...
namespace supermario
{
    public partial class mainWin : Form
    {
        ...
        private void InitializeGame()
        {
            ...
            if (picboxplayer != null)
            {
                ...
                try { picboxplayer.Image = Properties.Resources.הורדה; picboxplayer.BackColor = Color.Transparent; }
        ...
        private void CreateLongLevel()
        {
            CreateBrickGround();
            ClearPowerUps();
            foreach (var p in currentLevel) AddPlatform(p.X, p.Y, p.Width, p.Height);
            AddQuestionBlocks();
        ...
        private void AddQuestionBlocks()
        {
            ...
            for (int i = 0; i < xPos.Length && i < yPos.Length; i++)
            {
                ...
                Controls.Add(box);
                Label lbl = new Label { Text = "?", Font = new Font("Arial", 24, FontStyle.Bold), ForeColor = Color.Black, TextAlign = ContentAlignment.MiddleCenter, Size = new Size(50, 50), Location = box.Location, BackColor = Color.Transparent };
                Controls.Add(lbl);
        ...
        {
            foreach (var b in questionBlocks) { if (b.Visual != null) { Controls.Remove(b.Visual); b.Visual.Dispose(); } if (b.QuestionLabel != null) { Controls.Remove(b.QuestionLabel); b.QuestionLabel.Dispose(); } }
            questionBlocks.Clear();
        }

        private void MainWin_KeyDown(object sender, KeyEventArgs e) { if (e.KeyCode == Keys.D) moveRight = true; if (e.KeyCode == Keys.A) moveLeft = true; if (e.KeyCode == Keys.W || e.KeyCode == Keys.Space) jump = true; if (e.KeyCode == Keys.Enter && !gameManager.IsGameRunning) { gameManager.StartGame(); gameTimer.Start(); Text = $"Super Mario - Level {currentLevelNumber} - RUNNING"; } if (e.KeyCode == Keys.Escape) { gameTimer.Stop(); gameManager.EndGame(); Text = $"Super Mario - Level {currentLevelNumber} - PAUSED"; } }
        private void MainWin_KeyUp(object sender, KeyEventArgs e) { if (e.KeyCode == Keys.D) moveRight = false; if (e.KeyCode == Keys.A) moveLeft = false; if (e.KeyCode == Keys.W || e.KeyCode == Keys.Space) jump = false; }

        private void GameLoop(object sender, EventArgs e)
        {
            if (!gameManager.IsGameRunning) return;
            if (isDying) { HandleDeathAnimation(); return; }
        ...
        private void CheckPlatformCollisions()
        {
            ...
            foreach (var plat in platforms)
            {
                ...
                {
                    int top = playerRect.Bottom - platRect.Top;
                    if (top < 20 && top == Math.Min(Math.Min(playerRect.Right - platRect.Left, platRect.Right - playerRect.Left), Math.Min(top, platRect.Bottom - playerRect.Top)))
                    {
        ...
        private void HandleFallDamage()
        {
            ...
            if (!wasGroundedLastFrame && player.IsGrounded && !isDying)
            {
                ...
                if (fallDist > FALL_DAMAGE_THRESHOLD && canTakeFallDamage) { player.TakeDamage(1); canTakeFallDamage = false; if (player.Health <= 0) { isDying = true; deathTimer = 0f; } }
                maxFallStartY = 0;
            }
        ...
        private void RestartLevel() { gameManager.ResetGame(); cameraX = 0; isDying = false; wasGroundedLastFrame = false; canTakeFallDamage = true; isPlayerSuper = false; player.Respawn(new Point(100, 405)); player.IsGrounded = true; player.Health = 3; picboxplayer.Size = originalPlayerSize; picboxplayer.Location = player.Position; ClearPlatforms(); CreateLongLevel(); gameManager.StartGame(); gameTimer.Start(); }
        private void LoadNextLevel() { currentLevelNumber++; currentLevel = allLevels[currentLevelNumber - 1]; gameManager.ResetGame(); cameraX = 0; isDying = false; wasGroundedLastFrame = false; canTakeFallDamage = true; isPlayerSuper = false; player.Respawn(new Point(100, 405)); player.IsGrounded = true; player.Health = 3; picboxplayer.Size = originalPlayerSize; picboxplayer.Location = player.Position; ClearPlatforms(); CreateLongLevel(); Text = $"Super Mario - Level {currentLevelNumber}"; gameManager.StartGame(); gameTimer.Start(); }

        private void ClearPlatforms() { foreach (var p in platforms) { Controls.Remove(p.PictureBox); p.PictureBox.Dispose(); } platforms.Clear(); ClearPowerUps(); }
        ...
        private PlatformData[] GenerateRandomLevel(int numSections)
        {
            ...
            for (int i = 0; i < numSections; i++) sections.Add(ALL_SECTIONS[levelRandom.Next(ALL_SECTIONS.Length)]);
            foreach (var sec in sections)
            {
                ...
                if (sec.Length > 0) { int maxX = 0; foreach (var p in sec) if (p.X + p.Width > maxX) maxX = p.X + p.Width; xOff += maxX + 100; }
            }
    ...
    public class Mushroom { public Point Position { get; set; } public PictureBox Visual { get; set; } public bool IsCollected { get; set; } public Mushroom(Point pos, PictureBox visual) { Position = pos; Visual = visual; IsCollected = false; } }
    public class QuestionBlock { public Point Position { get; set; } public PictureBox Visual { get; set; } public Label QuestionLabel { get; set; } public bool IsHit { get; set; } public PowerUpType PowerUpInside { get; set; } public QuestionBlock(Point pos, PictureBox visual, Label label, PowerUpType powerUp) { Position = pos; Visual = visual; QuestionLabel = label; IsHit = false; PowerUpInside = powerUp; } }
}